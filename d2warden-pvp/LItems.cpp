...
int ClientID = pUnit->pPlayerData->pClientData->ClientID;
	...
	if(_stricmp(str,"#icreate")==0)
		...
		UnitAny* ptItem = D2Funcs::D2GAME_CreateItem(99,1,1,pUnit,Idx,pUnit->pGame,3,iQual,1,0,0);
		CreateItem sets;
		...
		//D2Funcs::D2GAME_DropItem(ptItem,pUnit->pPath->pRoom1,pUnit->pGame,pUnit,pUnit->pPath->xPos,pUnit->pPath->yPos);
		//D2Funcs::D2COMMON_SetCursorItem(pUnit->pInventory,0);
		UnitAny* pNew = D2Funcs::D2GAME_DupeItem(ptItem->pGame,ptItem,1);
		if(pNew)
		{
		...
		/*	pNew->pItemData->dwItemFlags.bInit=1;
			...
			D2Funcs::D2GAME_DropItem(pNew,pUnit->pPath->pRoom1,pUnit->pGame,pUnit,pUnit->pPath->xPos,pUnit->pPath->yPos);
		}
		...
		if(ptFound->ptClientData->InitStatus) D2Funcs::D2GAME_SendPacket(ptFound->ptClientData,aPacket,6);
		D2Funcs::D2COMMON_FlushItem(ptInv,i);
		D2Funcs::D2GAME_DeleteUnit(ptFound->ptGame,i);
		//D2Funcs::D2GAME_RemoveItem(ptFound->ptGame,ptFound->ptPlayer,i);
