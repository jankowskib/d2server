...
void __fastcall NODES_AssignNode(Game *ptGame, UnitAny *ptUnit, UnitNode* pChild)  // 0xEDF00
{
	...
	for (int n = 0; n < Warden::getInstance().wcfgMaxPlayers; ++n)
	{
		if (!pNodes[n]) {
			...
			{
				Log("NodesEX: nClients=%d <= Warden::getInstance().wcfgMaxPlayers=%d || z=%d < MaxPlayers=%d", ptGame->nClients, Warden::getInstance().wcfgMaxPlayers, n, Warden::getInstance().wcfgMaxPlayers);
			}
			break;
		}
	}
